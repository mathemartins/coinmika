...
from coinmika.utils import get_mnemonic_seed_phrase, create_ledger_account, get_address, activate_transaction_alert, \
    get_account_list
from wallet.models import BitcoinMnemonics, LedgerAccount, Address
...
class ListWallets(APIView):
    ...
    def get(self, *args, **kwargs):
        ...
        data = self.request.data
        try:
            account_list: dict = get_account_list(customer_id=data.get("customerId"))
            return Response(
                {
                    "message": "Successful",
                    "success": True,
